...
#pragma once
	...
	extern "C" __declspec(dllexport) int __stdcall  ExtIoGetSrates(int idx, double* samplerate);  // fill in possible samplerates
	extern "C" __declspec(dllexport) int __stdcall  ExtIoGetActualSrateIdx(void);               // returns -1 on error
	extern "C" __declspec(dllexport) int __stdcall  ExtIoSetSrate(int idx);                    // returns != 0 on error
	extern "C" __declspec(dllexport) long  __stdcall  ExtIoGetBandwidth(int srate_idx);       // returns != 0 on error
